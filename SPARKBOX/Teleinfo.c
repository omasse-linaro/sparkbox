 ...
 * Nom:			TELEINFO.C
 * Fonction :	Gestion des données Téléinfo du compteur électronique
 *
 *  SARL ADTEK
 **********************************************************************************************************
...
void Process_Tele( void )
  {
    ...
    {   
                ...
                switch ( Tele_State[portTele] )
                {
                    ...
                    case TELE_RCV :
                        if ( tempRS[i] == TELE_CR )                 // Recherche fin de groupe
                        {
							...
							Compteur[portTele].last_cpt1s	= cpt1s; // Horodatage pour détecter un compteur qui disparait
        ...
        if ((cpt1s - Compteur[portTele].last_cpt1s) > 15) // Pas de Téléinfo depuis plus de 10s
        {
...
//*******************************************************
 void Process_Calcul_Index(unsigned char port)
 {
	...
	// On viens recréer en ascii le cumul des index
...
void Process_Calcul_Teleinfo(unsigned char port)
{
	...
	static unsigned int index 		= 0;
 	...
 	if (Compteur[port].etat == CAPTEUR_OK)
 	{
	 	...
	 	{
			if (cpt1s > 10) // Passé a 10 secondes pour faire le calcul avant Init_Jour au démarrage
		 	{
			 	Process_Calcul_Index(port);
			 	// Préparation du prochain passage dans une minute !
	 	...
	 	else 
	 	{
			if (cpt1s >= Compteur[port].calc_cpt1s)
			{
				...
				{
					// On calcule combien de W sont passés
					index = Compteur[port].calc_index - index_sav[port];
					// Calcul de la puissance sur l'échelle de temps	
	...
	// Problème sur le Téléinfo, on remet à zéro les calculs correspondants
	else
	{
		Compteur[port].calc_puis 		= 0;
		Compteur[port].calc_puis_s[0]	= 0;
		// On ne touche pas à la sauvegarde de l'index ni à son cpt1s car on pourra dés le retour calculer la puissance
	}
}
